...
from utils.clip_fortrain import ClipLoss
...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, names, use_dfl=True):
        ...
        self.use_dfl = use_dfl
        self.clip_loss = ClipLoss(class_names=names)
    ...
    ###########################################################
    def __call__(self, p, targets, img, epoch=0): # img as arg
        print("---------------- debug --------------")
        loss = torch.zeros(4, device=self.device)  # box, cls, dfl
        feats = p[1][0] if isinstance(p, tuple) else p[0]
        feats2 = p[1][1] if isinstance(p, tuple) else p[1]
        print("[DEBUG] feats shape: ", feats[0].shape)
        print("[DEBUG] feats2 shape: ", feats2[0].shape)
        ...
        print("[DEBUG] pred scores shape: ", pred_scores.shape)
        print("[DEUBG] pred bboxes shape: ", pred_bboxes.shape)
        ...
        print("[DEBUG] target scores shape: ", target_scores.shape)
        ...
        loss[3] = self.clip_loss((pred_bboxes.detach() * stride_tensor).type(gt_bboxes.dtype), pred_scores, img)
